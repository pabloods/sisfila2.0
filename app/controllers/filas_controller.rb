  ...
  def gera_estatisticas_tela
    ...
    if (@fila.posicao <= 2) || (@fila.npatendidas <= 2) 
      redirect_to @fila
        ...
        # Tempo médio de espera na fila (MANUAL)
        # @aux = 0
        # @todasvagas = Vaga.where(fila_id: @fila.id).where("temponafila > 0").all
        #  for a in (1 .. Vaga.where(fila_id: @fila.id).where("temponafila > 0").all.size) do      
        #   @aux = @aux + @todasvagas[a-1].temponafila
        #  end
        # @aux = @aux / Vaga.where(fila_id: @fila.id).where("temponafila > 0").all.size
        # @tmespera = ActiveSupport::Duration.build(@aux).parts
        # Fila.where(id: @fila.id).update(tempomedioespera: @aux) 

        

        # Tempo médio de espera na fila
        @tmespera = ActiveSupport::Duration.build(Fila.where(id: @fila.id).last.tempomedioespera).parts
